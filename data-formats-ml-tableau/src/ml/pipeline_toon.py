...
def run_toon_pipeline() -> Dict[str, float]:
    """
    Train and evaluate a model using the TOON representation.
    Returns a metrics dict including timing breakdown, cross validation stats,
    and encoded feature space stats.
    """
    path = DATA_FORMATTED / TOON_FILENAME
    df = toon_to_canonical_df(path)
...
def main():
    metrics = run_toon_pipeline()
    print("TOON pipeline metrics:")
